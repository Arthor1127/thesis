...
class RouteMeanderGrounded(RouteMeander):
    ...
    def make_ground_straps(self, pts: np.ndarray):
        ...
        # spans from the outer edge of one gap, across the trace, to the outer edge
        # of the other gap - i.e. fully shorts center conductor to both ground planes
        strap_length = self.p.trace_width + 2 * self.p.trace_gap
