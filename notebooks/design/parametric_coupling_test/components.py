...
class Component:
    ...
    def impedance(self):
        ...
        content to derive this from. See Resonator.impedance() and
        Qubit.impedance() -- the formula is genuinely different per type
        (Z=sqrt(L/C) for a resonator, Z_q=sqrt(L_J/C_Sigma) for a qubit).
        """
        raise NotImplementedError(
...
class Resonator(Component):
    ...
    def impedance(self):
        """Characteristic impedance Z = sqrt(L/C), via
        DesignHelper.compute_resonator_impedance. Returned in
        resistance_units. Requires the same two-of-three (omega_r, L, C)
        that zpfs() uses internally -- no additional parameters needed.
        """
...
class Qubit(Component):
    ...
    def impedance(self):
        ...
        Z_q = sqrt(L_J / C_Sigma), via DesignHelper.compute_transmon_impedance.
        Returned in resistance_units. This is the harmonic-approximation
        impedance that sets the scale for the charge/flux ZPFs -- the
        anharmonicity doesn't appear here (it appears in the spectrum,
        not the ZPF magnitudes at leading order).
        """
        return self.dh.compute_transmon_impedance(self.EJ, self.EC)
