"""Quick check: mesh-only (no AMR, no solve) with the new readout-widened/
qubit-narrowed taper sizes, to gauge base problem size before committing
to a full AMR job. Mirrors run_amr.py's mesh setup exactly."""
import os
import sys
import time
...
OUTDIR = sys.argv[1] if len(sys.argv) > 1 else "runs/check_base_mesh2"
...
eigen_sim = PALACE_Eigenmode_Simulation(
    name="check2", metal_design=design, sim_parent_directory=OUTDIR,
...
eigen_sim.fine_mesh_components(
    ["readout_1"], min_size=4e-6, max_size=150e-6, taper_dist_min=40e-6, metals_only=False,
)
eigen_sim.fine_mesh_components(
...
msh_path = os.path.join(OUTDIR, "check2", "check2.msh")
