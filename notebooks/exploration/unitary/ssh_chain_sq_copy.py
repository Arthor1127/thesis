...
class PHOTONICChain:
    def __init__(self, N, Omega_C, Chi_C, omega_r, gamma, fock_photon,
                 Omega_J, Chi_J, PBC=False, coeff_tol=1e-9, zero_tol=1e-12):
        ...
        # ------------------------------------------------------------------
        # Capacitive Nambu matrices (identical to ssh_chain_single)
        # ------------------------------------------------------------------
        T = np.zeros((n, n))
        for j in range(n - 1):
        ...
        # ------------------------------------------------------------------
        # Josephson Nambu matrices (identical to ssh_chain_single)
        # ------------------------------------------------------------------
        TJ = np.zeros((n, n))
        for j in range(n - 1):
        ...
        _, self.U, self.V = engine.diagonalize_quadratic(H_C_for_diag)

        # Reconstruct the full pseudo-unitary P (same structure as ssh_chain_single)
        u_part = self.U   # shape (n, n)
        v_part = self.V   # shape (n, n)
        U_V    = np.vstack([u_part, v_part])
        J_U_V  = np.vstack([v_part, u_part])   # J acts as swap for real vecs

        self.P         = np.hstack([U_V, J_U_V])
        self.inverse_P = np.linalg.solve(self.P, np.eye(self.P.shape[0]))

        # ------------------------------------------------------------------
        # Conjugate both Hamiltonians into the Bogoliubov basis
        # (exactly as ssh_chain_single does with P.T.conj() @ M @ P)
        # ------------------------------------------------------------------
    ...
    def build_HC_fock(self, states, n_total):
        ...
        from the Bogoliubov transformation convention.
        No zero-point energy / commutation constants included.
        """
        n = 2 * self.N
        energies = np.diag(self.H_C)[:n].real   # (2N,) positive-branch energies

        diag_vals = states @ energies             # (n_total,) — one energy per state
    ...
    def build_HJ_fock(self, states, state_index, n_total):
        ...
        return self.engine.build_sparse_operator_matrix(op, basis)
    ...
    def schrieffer_wolff_correction(self, eps_denom=1e-6):
        ...
        energies_vec = 2.0 * np.diag(self.H_C)[:n].real   # (2N,)
            ...
            for n1 in range(d_ph):
                for n2 in range(d_ph):
                    for n_mid in range(d_ph):
                        ...
                        denom1 = (n1 - n_mid) * omega_r + E_X[:, None] - E_r[None, :]
                        denom2 = (n_mid - n2) * omega_r + E_r[:, None] - E_X[None, :]
                        ...
                        term1 = (V_r * inv1.T).T @ V_r.conj()
                        term2 = V_r.T @ (V_r.conj() * inv2)
    ...
    def new_to_old_fock(self, new_fock_state, threshold=0.99):
        ...
        U_t = self.P[:n, :n].T
        V_t = -self.P[n:, :n].T
        state = Superposition({tuple(int(x) for x in new_fock_state): 1.0 + 0j})

        result = self.engine.transform_state(state, U_t, V_t, direction='new_to_old',
                                              base_state=base_state)
        return self.engine.trim_state(result.states, threshold=threshold)
    ...
    def old_to_new_fock(self, old_fock_state, threshold=0.99):
        ...
        state = Superposition({tuple(int(x) for x in old_fock_state): 1.0 + 0j})
    ...
    def warm_old_basis_cache(self):
        ...
        for state in self.fock_states:
            ...
            if key not in self._new_to_old_cache:
                self._new_to_old_cache[key] = self.new_to_old_fock(np.array(state, dtype=int))
        ...
        self._C_matrix = C
        self._old_states_arr = np.array(sorted_states, dtype=float)
